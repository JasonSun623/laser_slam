...
void SlamGMapping::init()
{
  ...
  {
      if(nh_.getParam("/sick_tim561/range_max",range_max))
      {
          maxRange_ = range_max;
      }
      else
...
void SlamGMapping::startLiveSlam()
{
  ...
  map_build_filter_pub_ = node_.advertise<gmapping::map_filter>("map_build_filter", 1, true);
  ...
  state_client_ = node_.serviceClient<localization::localization_state>("/localization/get_localizaiton_state");
  register_client_ = node_.serviceClient<localization::localization_register>("/localization/register_localizaiton_ok");
  registerOK();
}
...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  if(!isGmappingState())
  {
    ROS_DEBUG("robot is not in gmapping state now");
    got_first_scan_ = false;
    return;
  }
  laser_count_++;
  ...
  if(!got_first_scan_)
  {
    ...
    map_build_filter_.data_free.resize(map_.map.info.width * map_.map.info.height);
    map_build_filter_.data_obstacle.resize(map_.map.info.width * map_.map.info.height);
    if(!initMapper(*scan))
      return;
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
  ...
  for(int x=0; x < smap.getMapSizeX(); x++)
  {
    for(int y=0; y < smap.getMapSizeY(); y++)
    {
      ...
      double occ_cell = smap.cell(p);
      if( occ_cell> occ_thresh_)
      {
        filter_obstacle_size++;
      }
      else if(occ_cell <= occ_thresh_ && occ_cell >= 0)
      {
        filter_free_size++;
      }
    }
  ...
  for(int x=0; x < smap.getMapSizeX(); x++)
  {
    for(int y=0; y < smap.getMapSizeY(); y++)
    {
      ...
      else if(occ > occ_thresh_)
      {
        ...
        map_build_filter_.data_obstacle[filter_obstacle_count] = MAP_IDX(map_.map.info.width, x, y);
        filter_obstacle_count++;
      }
      else
      {
        map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = 0;
        map_build_filter_.data_free[filter_free_count] = MAP_IDX(map_.map.info.width, x, y);
        filter_free_count++;
      }
    }
  }
  ...
  if(isGmappingState())
  {
     ...
     map_build_filter_.header = map_.map.header;
     map_build_filter_.info = map_.map.info;
