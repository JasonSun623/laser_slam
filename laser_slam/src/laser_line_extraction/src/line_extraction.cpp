...
///////////////////////////////////////////////////////////////////////////////
// GIve a point ,in in before computing lines
///////////////////////////////////////////////////////////////////////////////
bool LineExtraction::isPointInLine(double th, double range, const std::vector<Line> &lines)
{
  bool rlt = false;
  double x = cos(th) * range;
  double y = sin(th) * range;
  double x0, y0, x1, y1;
  double len_0_1, len_0_x, len_x_1;
  for (std::vector<Line>::const_iterator cit = lines.begin(); cit != lines.end(); ++cit)
  {
    ...
    x1 = cit->getEnd()[0];
    y1 = cit->getEnd()[1];
    len_0_1 = hypot(x1-x0,y1-y0);
    len_0_x = hypot(x-x0,y-y0);
    len_x_1 = hypot(x1-x,y1-y);
    if(std::abs(len_0_x + len_x_1 - len_0_1) < params_.max_distance_in_line)
    {
      rlt = true;
      break;
    }
...
///////////////////////////////////////////////////////////////////////////////
void LineExtraction::cacheData(const sensor_msgs::LaserScan::ConstPtr &scan_msg)
{
  ...
  setCachedData(bearings, cos_bearings, sin_bearings, indices);
  ROS_DEBUG("Data has been cached.");
}

sensor_msgs::LaserScan LineExtraction::getScanLine(const sensor_msgs::LaserScan::ConstPtr &scan_msg)
{
  boost::mutex::scoped_lock scan_lock(scan_mutex_);
  // cache data
  cacheData(scan_msg); 
  std::vector<double> scan_ranges_doubles(scan_msg->ranges.begin(), scan_msg->ranges.end());
  setRangeData(scan_ranges_doubles);
  // extract the lines
  ...
  if(lines.empty())
  {
      ROS_ERROR("extractLines is empty");
      return *scan_msg;
  }

  sensor_msgs::LaserScan scan_line(*scan_msg);
  unsigned int i = 0;
  double angle = 0.0;
  double range = 0.0;
  for (double b = scan_msg->angle_min; b <= scan_msg->angle_max; b += scan_msg->angle_increment)
  {  
    ...
    if(!isPointInLine(angle,range,lines))
    {
      scan_line.ranges[i] = 0.0;
    }
    i++;
  }   
  return scan_line;
...
void LineExtraction::setMaxDistanceInLine(double value)
{
  params_.max_distance_in_line = value;
}
