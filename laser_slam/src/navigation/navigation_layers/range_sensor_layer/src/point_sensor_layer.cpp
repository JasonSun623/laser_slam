...
PLUGINLIB_EXPORT_CLASS(range_sensor_layer::PointTimeSensorLayer, costmap_2d::Layer)
...
PointTimeSensorLayer::PointTimeSensorLayer() {}

void PointTimeSensorLayer::onInitialize()
{
  ...
  for (unsigned int i = 0; i < topic_names.size(); i++)
  {
    ...
    else
    {
      ...
      if (input_sensor_type == VARIABLE)
        processRangeMessageFunc_ = boost::bind(&PointTimeSensorLayer::processVariableRangeMsg, this, _1);
      else if (input_sensor_type == FIXED)
        processRangeMessageFunc_ = boost::bind(&PointTimeSensorLayer::processFixedRangeMsg, this, _1);
      else if (input_sensor_type == ALL)
        processRangeMessageFunc_ = boost::bind(&PointTimeSensorLayer::processRangeMsg, this, _1);
      else
      {
        ROS_ERROR(
      ...
      range_subs_.push_back(nh.subscribe(topic_name, 100, &PointTimeSensorLayer::bufferIncomingRangeMsg, this));
  ...
  dynamic_reconfigure::Server<range_sensor_layer::RangeSensorLayerConfig>::CallbackType cb = boost::bind(
      &PointTimeSensorLayer::reconfigureCB, this, _1, _2);
  dsrv_->setCallback(cb);
  global_frame_ = layered_costmap_->getGlobalFrameID();
}


double PointTimeSensorLayer::gamma(double theta)
{
...
double PointTimeSensorLayer::delta(double phi)
{
    return 1;// - (1+tanh(2*(phi-phi_v_)))/2;
}

double PointTimeSensorLayer::sensor_model(double r, double phi, double theta)
{
...
void PointTimeSensorLayer::reconfigureCB(range_sensor_layer::RangeSensorLayerConfig &config, uint32_t level)
{
  phi_v_ = config.phi;
...
void PointTimeSensorLayer::bufferIncomingRangeMsg(const range_sensor_layer::sonar_msgsConstPtr& range_message)
{
  boost::mutex::scoped_lock lock(range_message_mutex_);
  range_msgs_buffer_.push_back(*range_message);
}

void PointTimeSensorLayer::updateCostmap()
{
  ...
  for (std::list<range_sensor_layer::sonar_msgs>::iterator it = time_range_msgs_buffer_.begin();
      it != time_range_msgs_buffer_.end(); )
  {
      if(ros::Time::now() - it->header.stamp < ros::Duration(keep_time_))
      {
          processRangeMessageFunc_(*it);
          it++;
      }
      else
      {
          it = time_range_msgs_buffer_.erase(it);
      }
  }
}

void PointTimeSensorLayer::processRangeMsg(range_sensor_layer::sonar_msgs& range_message)
{
...
void PointTimeSensorLayer::processFixedRangeMsg(range_sensor_layer::sonar_msgs& range_message)
{
...
void PointTimeSensorLayer::processVariableRangeMsg(range_sensor_layer::sonar_msgs& range_message)
{
...
void PointTimeSensorLayer::updateCostmap(sensor_msgs::Range& range_message, bool clear_sensor_cone)
{
...
void PointTimeSensorLayer::update_cell(double ox, double oy, double ot, double r, double nx, double ny, bool clear)
{
  unsigned int x, y;
...
void PointTimeSensorLayer::clearCostmap()
{
...
void PointTimeSensorLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x,
...
void PointTimeSensorLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i,
                                          int max_j)
{
  if (!enabled_)
    return;
...
void PointTimeSensorLayer::reset()
{
...
void PointTimeSensorLayer::deactivate()
{
  range_msgs_buffer_.clear();
}

void PointTimeSensorLayer::activate()
{
  range_msgs_buffer_.clear();
}
