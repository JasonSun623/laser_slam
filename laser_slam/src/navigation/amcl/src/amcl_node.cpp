...
class AmclNode
{
  ...
  private:
    ...
    void currentPoseReceived(const nav_msgs::Odometry& msg);
    ...
    double getRangeWeight(double range , double range_max);
  ...
  //beam skip related params
    ...
    bool tf_broadcast_;
    bool use_range_weight_;
    double bad_laser_range_min_, bad_laser_range_max_, bad_laser_range_rate_;
    bool bad_laser_check_;
    ...
    bool checkBadLaserData(const sensor_msgs::LaserScanConstPtr& laser_scan);
    volatile bool running_initial_pose_;
};
        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("laser_max_beams", max_beams_, 100);
  ...
  private_nh_.param("odom_alpha1", alpha1_, 1.0);
  private_nh_.param("odom_alpha2", alpha2_, 0.2);
  private_nh_.param("odom_alpha3", alpha3_, 0.5);
  private_nh_.param("odom_alpha4", alpha4_, 0.05);
  private_nh_.param("odom_alpha5", alpha5_, 0.1);
  ...
  private_nh_.param("laser_z_hit", z_hit_, 0.98);
  private_nh_.param("laser_z_short", z_short_, 0.05);
  private_nh_.param("laser_z_max", z_max_, 0.05);
  private_nh_.param("laser_z_rand", z_rand_, 0.02);
  private_nh_.param("laser_sigma_hit", sigma_hit_, 0.1);
  ...
  private_nh_.param("update_min_a", a_thresh_, 0.15);
  ...
  private_nh_.param("use_range_weight", use_range_weight_, false);
  private_nh_.param("bad_laser_check", bad_laser_check_, false);
  private_nh_.param("bad_laser_range_min", bad_laser_range_min_, 0.3);
  private_nh_.param("bad_laser_range_max", bad_laser_range_max_, 10.0);
  private_nh_.param("bad_laser_range_rate", bad_laser_range_rate_, 0.6);

  ROS_INFO("bad_laser_check_: %d", bad_laser_check_);
  ROS_INFO("bad_laser_range_min_: %.3f", bad_laser_range_min_);
  ROS_INFO("bad_laser_range_max_: %.3f", bad_laser_range_max_);
  ROS_INFO("bad_laser_range_rate_: %.3f", bad_laser_range_rate_);
  ...
  current_pose_sub_ = nh_.subscribe("current_pose", 2, &AmclNode::currentPoseReceived, this);

  state_client_ = nh_.serviceClient<localization::localization_state>("/localization/get_localizaiton_state");

  register_client_ = nh_.serviceClient<localization::localization_register>("/localization/register_localizaiton_ok");
...
int AmclNode::registerOK()
{
   localization::localization_register srv;
   srv.request.name = "amcl";
   register_client_.call(srv);
   ROS_DEBUG("amcl init ok");
   return 0;
}
...
AmclNode::mapReceived(const nav_msgs::OccupancyGridConstPtr& msg)
{
  if(!isNavState())
  {
    ROS_DEBUG("robot is not in navgation state now");
    return;
  }
...
AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
{
  ...
  updatePoseFromServer();
  pf_vector_t pf_init_pose_mean = pf_vector_zero();
  /*double max_x_map = (msg.info.width)*(msg.info.resolution)/2.0;
  double max_y_map = (msg.info.height)*(msg.info.resolution)/2.0;
  ROS_DEBUG("amcl map_size x %.3f, y %.3f, current pose x %.3f,current pose y %.3f",
           max_x_map, max_y_map,
           current_pose_.pose.pose.position.x,current_pose_.pose.pose.position.y);
  if(!std::isnan(tf::getYaw(current_pose_.pose.pose.orientation))
      && (std::abs(current_pose_.pose.pose.position.x) <= max_x_map)
      && (std::abs(current_pose_.pose.pose.position.y) <= max_y_map))
  {
     pf_init_pose_mean.v[0] = current_pose_.pose.pose.position.x;
     pf_init_pose_mean.v[1] = current_pose_.pose.pose.position.y;
     pf_init_pose_mean.v[2] = tf::getYaw(current_pose_.pose.pose.orientation);
  }
  else
  {
  ...
  applyInitialPose();
  ROS_INFO("amcl handle map msg finish");
}
...
/*
*  get localization state from service
*/
bool
AmclNode::isNavState()
{
   localization::localization_state srv;
   state_client_.call(srv);
...
bool
AmclNode::checkBadLaserData(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
    int bad_range_count = 0;
    for(auto range: laser_scan->ranges)
    {
        if(range < bad_laser_range_min_ || range > bad_laser_range_max_)
        {
            bad_range_count++;
        }
    }

    if(bad_range_count/laser_scan->ranges.size() > bad_laser_range_rate_)
    {
        if(latest_tf_valid_)
        {
          if (tf_broadcast_ == true && isNavState())
          {
...
/*
*  handle laser info when received
*  tf laser->base link ->odom
*  Use the action data to update the filter
*  updateSensor weights
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(bad_laser_check_)
  {
    if(running_initial_pose_)
    {
      ROS_DEBUG("set running_initial_pose_ false.");
      running_initial_pose_ = false;
    }
    else if(!checkBadLaserData(laser_scan))
    {
      ROS_DEBUG("got bad laser data.");
      return;
    }
  }
  ...
  if(lasers_update_[laser_index])
  {
    ...
    ldata.range_weight = new double[ldata.range_count];
    ROS_ASSERT(ldata.range_weight);
    for(int i=0;i<ldata.range_count;i++)
    {
      ...
      ldata.ranges[i][1] = angle_min +
              (i * angle_increment);
      if(use_range_weight_)
      {
        ldata.range_weight[i] = getRangeWeight(laser_scan->ranges[i],ldata.range_max);
      }
      else
      {
        ldata.range_weight[i] = 1;
      }
      ROS_DEBUG("amcl range_weight :%.3f",ldata.range_weight[i]);
    }
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0 &&
        !std::isnan(hyps[max_weight_hyp].pf_pose_mean.v[0]) &&
        !std::isnan(hyps[max_weight_hyp].pf_pose_mean.v[1]) &&
        !std::isnan(hyps[max_weight_hyp].pf_pose_mean.v[2]))
    {
...
double
AmclNode::getRangeWeight(double range , double range_max)
{
  double weight;
  weight = (range/range_max);
  return weight;
}
...
void
AmclNode::currentPoseReceived(const nav_msgs::Odometry& msg)
{
   current_pose_ = msg;
   ROS_DEBUG("amcl current location %.3f, %.3f",
              current_pose_.pose.pose.position.x,current_pose_.pose.pose.position.y);
}
