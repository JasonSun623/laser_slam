...
MultiMapper::MultiMapper()
{
	...
	mapperNode.param("min_map_size", mMinMapSize, 20);
	...
	map_build_filter_pub_ = robotNode.advertise<gmapping::map_filter>("map_build_filter", 1, true);
	...
	stateClient = robotNode.serviceClient<localization::localization_state>("/localization/get_localizaiton_state");
	register_client_ = robotNode.serviceClient<localization::localization_register>("/localization/register_localizaiton_ok");
	...
	if(mapperNode.getParam("use_scan_matching", param_b))
		mMapper->SetParameters("UseScanMatching", param_b);
		
	if(mapperNode.getParam("use_scan_barycenter", param_b))
		mMapper->SetParameters("UseScanBarycenter", param_b);
		
	if(mapperNode.getParam("minimum_travel_distance", param_d))
		mMapper->SetParameters("MinimumTravelDistance", param_d);
	
	if(mapperNode.getParam("minimum_travel_heading", param_d))
		mMapper->SetParameters("MinimumTravelHeading", param_d);
		
	if(mapperNode.getParam("scan_buffer_size", param_i))
		mMapper->SetParameters("ScanBufferSize", param_i);
		
	if(mapperNode.getParam("scan_buffer_maximum_scan_distance", param_d))
		mMapper->SetParameters("ScanBufferMaximumScanDistance", param_d);
		
	if(mapperNode.getParam("use_response_expansion", param_b))
		mMapper->SetParameters("UseResponseExpansion", param_b);
		
	if(mapperNode.getParam("distance_variance_penalty", param_d))
		mMapper->SetParameters("DistanceVariancePenalty", param_d);
		
	if(mapperNode.getParam("minimum_distance_penalty", param_d))
		mMapper->SetParameters("MinimumDistancePenalty", param_d);
		
	if(mapperNode.getParam("angle_variance_penalty", param_d))
		mMapper->SetParameters("AngleVariancePenalty", param_d);
		
	if(mapperNode.getParam("minimum_angle_penalty", param_d))
		mMapper->SetParameters("MinimumAnglePenalty", param_d);
		
	if(mapperNode.getParam("link_match_minimum_response_fine", param_d))
		mMapper->SetParameters("LinkMatchMinimumResponseFine", param_d);
		
	if(mapperNode.getParam("link_scan_maximum_distance", param_d))
		mMapper->SetParameters("LinkScanMaximumDistance", param_d);
		
	if(mapperNode.getParam("correlation_search_space_dimension", param_d))
		mMapper->SetParameters("CorrelationSearchSpaceDimension", param_d);
		
	if(mapperNode.getParam("correlation_search_space_resolution", param_d))
		mMapper->SetParameters("CorrelationSearchSpaceResolution", param_d);
		
	if(mapperNode.getParam("correlation_search_space_smear_deviation", param_d))
		mMapper->SetParameters("CorrelationSearchSpaceSmearDeviation", param_d);
		
	if(mapperNode.getParam("coarse_search_angle_offset", param_d))
		mMapper->SetParameters("CoarseSearchAngleOffset", param_d);
		
	if(mapperNode.getParam("fine_search_angle_offset", param_d))
		mMapper->SetParameters("FineSearchAngleOffset", param_d);
		
	if(mapperNode.getParam("coarse_angle_resolution", param_d))
		mMapper->SetParameters("CoarseAngleResolution", param_d);
		
	if(mapperNode.getParam("loop_search_space_dimension", param_d))
		mMapper->SetParameters("LoopSearchSpaceDimension", param_d);
		
	if(mapperNode.getParam("loop_search_space_resolution", param_d))
		mMapper->SetParameters("LoopSearchSpaceResolution", param_d);
		
	if(mapperNode.getParam("loop_search_space_smear_deviation", param_d))
		mMapper->SetParameters("LoopSearchSpaceSmearDeviation", param_d);
		
	if(mapperNode.getParam("loop_search_maximum_distance", param_d))
		mMapper->SetParameters("LoopSearchMaximumDistance", param_d);
		
	if(mapperNode.getParam("loop_match_minimum_chain_size", param_i))
		mMapper->SetParameters("LoopMatchMinimumChainSize", param_i);
		
	if(mapperNode.getParam("loop_match_maximum_variance_coarse", param_d))
		mMapper->SetParameters("LoopMatchMaximumVarianceCoarse", param_d);
		
	if(mapperNode.getParam("loop_match_minimum_response_coarse", param_d))
		mMapper->SetParameters("LoopMatchMinimumResponseCoarse", param_d);
		
	if(mapperNode.getParam("loop_match_minimum_response_fine", param_d))
	...
	map_build_filter_.data_obstacle.resize(100);
	map_build_filter_.data_free.resize(100);
...
{	
	...
	if(!isKartoState())
	{
		ROS_DEBUG("robot is not in karto state now");
		gotFirstScan = false;
		return;
	}
	...
	if(mState == ST_MAPPING)
	{
		...
		try
		{
			mTransformListener.lookupTransform(mOdometryFrame, mLaserFrame, scan->header.stamp, tfPose);
		}
		catch(tf::TransformException e)
		{
			try
			{
				mTransformListener.lookupTransform(mOdometryFrame, mLaserFrame, ros::Time(0), tfPose);
			}
			catch(tf::TransformException e)
			{
				ROS_ERROR("Failed to compute odometry pose, skipping scan (%s)", e.what());
				return;
			}
		}
		...
		if(success)
		{
			...
			try
			{
				mTransformListener.transformPose(mOdometryFrame, tf::Stamped<tf::Pose>(map_in_robot, ros::Time(0) /*scan->header.stamp*/, mLaserFrame), map_in_odom);
			}
			catch(tf::TransformException e)
			{
				ROS_WARN("Transform from %s to %s failed! (%s)", mLaserFrame.c_str(), mOdometryFrame.c_str(), e.what());
				ok = false;
			}
			if(ok) 
			{
...
bool MultiMapper::sendMap()
{
	...
	//gavin 2018-6-23 for publish map info to artemis
	int filter_obstacle_size = 0;
    int filter_free_size = 0;
	for(int x=0; x < mGridMap.info.width; x++)
	{
	  for(int y=0; y < mGridMap.info.height; y++)
	  {
		char occ_cell = mGridMap.data[x+y*mGridMap.info.width];
		if(100 == occ_cell )
		{
	...
	ROS_INFO("mGridMap obstacle and free:%d,%d",filter_obstacle_size,filter_free_size);
	map_build_filter_.data_obstacle.resize(filter_obstacle_size);
	map_build_filter_.data_free.resize(filter_free_size);
	...
	for(int x=0; x < mGridMap.info.width; x++)
	{
	  for(int y=0; y < mGridMap.info.height; y++)
	  {
		...
		if(100 == occ_cell )
		{
		  map_build_filter_.data_obstacle[filter_obstacle_count] = x+y*mGridMap.info.width;
		  filter_obstacle_count++;
		}
		else if(0 == occ_cell)
		{
		  map_build_filter_.data_free[filter_free_count] = x+y*mGridMap.info.width;
		  filter_free_count++;
		}
	  }
	}
    mGridMap.header.stamp = ros::Time::now();
    mGridMap.header.frame_id = "map";
    if(isKartoState())
	{
		map_build_filter_.header = mGridMap.header;
        map_build_filter_.info = mGridMap.info;
        map_build_filter_pub_.publish(map_build_filter_);
	}
...
bool MultiMapper::updateMap()
{
		...
		mGridMap.info.origin.position.y != offset.GetY())
	{
		...
		mGridMap.info.height = height;
		mGridMap.info.origin.orientation.x = 0.0;
    	mGridMap.info.origin.orientation.y = 0.0;
		mGridMap.info.origin.orientation.z = 0.0;
		mGridMap.info.origin.orientation.w = 1.0;
...
bool MultiMapper::isKartoState()
{
   localization::localization_state srv;
   stateClient.call(srv);
   //ROS_INFO("slam_state :%s",srv.response.state.c_str());
   return (0 == srv.response.state.compare("karto"));
}

int MultiMapper::registerOK()
{
   localization::localization_register srv;
   srv.request.name = "nav2d_karto";
   register_client_.call(srv);
   ROS_DEBUG("nav2d_karto init ok");
   return 0;
}
...
{
	boost::mutex::scoped_lock lock(mapToOdomMutex);
	if(mState == ST_MAPPING && isKartoState())
	{
		//mTransformBroadcaster.sendTransform(tf::StampedTransform (mOdometryOffset, ros::Time::now() , mOffsetFrame, mOdometryFrame));
		mTransformBroadcaster.sendTransform(tf::StampedTransform (mMapToOdometry, ros::Time::now() , mMapFrame, mOdometryFrame));
	}
}
